rus.bar.plt <- function(x, v = NULL){ # Bar Plot of Stocks Returns
  
  for (n in 1:ncol(x)){ s <- x[,n] # Clean data
  
    security <- diff(log(s))[-1]
  ...
  axis(side = 1, at = seq(mn, mx, by = 10), las = 2) # configure x axis
  
  box() # Make borders for plot
}
rus.bar.plt(rus.df1) # Test

...
rus.bar.plt.stack.dividend.sector <- function(x, portion = F, c = "Roubles"){ 
  ...
  for (n in 1:length(rownames(D))){ # Get sector info from Morningstar website
  ...
  if (isTRUE(portion)){ # Plot showing stakes of dividends for each month
    ...
    ggplot(i, aes(x = Date, y = Quantity, fill = Stock)) + theme_minimal() +
      ...
      labs(title="Stacked Bar Plot of Portfolio Dividends by Sector",
           x = "Months", y = sprintf("Amount in %s", c), fill = "Sectors") +
      scale_fill_manual(values = C) }
}
rus.bar.plt.stack.dividend.sector(rus.test.df, portion = F) # Test

...
rus.dataframe.builder <- function(x, y){ 
  ...
  for (m in seq(x[,1])){ # For each asset
    ...
    p <- as.timeSeries(rus.df1[,v]) # time series 
    
    rownames(p) <- rownames(y) # Assign dates
    
    colnames(p) <- v # Assign tickers
    
    # Subset dates from data set and transform them into a date format
    dates_fr_yh <- as.Date(rownames(as.timeSeries(y)))
...
rus.portfolio.df <- rus.dataframe.builder(rus.nest.df, rus.df1) # Test
